...
/**
 * RegexFind.java - Streamsets expression language function for java regex find method
 * @author Michael Fox
 * @version 1.0
 */
...
public class RegexFind {
    ...
    @ElFunction(
            prefix = "regex",
            name = "find",
            description = "Returns true if the regex is found in the string"
    )
    public static boolean find(
            @ElParam("inString") String inString, @ElParam("regex") String regex
    ) {
        try {
            return Pattern.compile(regex).matcher(inString).find();
        } catch (Exception e) {
            return false;
        }
    }
